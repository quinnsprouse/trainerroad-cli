...
async function maybeLogin(page) {
  ...
  const looksLoggedOut =
    currentUrl.includes("login") || currentUrl.includes("signin") || hasPassword
...
const urlList = Array.from(seenUrls).toSorted((a, b) => String(a).localeCompare(String(b)))
const apiUrlList = Array.from(new Set(apiRecords.map((record) => record.url))).toSorted((a, b) =>
  a.localeCompare(b),
)
