...
function toCatalog(profilesByZone) {
  ...
  return rows.map((zone) => ({
    ...
    profiles: (Array.isArray(zone.workoutProfileOptions) ? zone.workoutProfileOptions : []).map(
      (profile) => ({
...
function applyLocalFilters(records, query) {
  return records.filter((item) => {
    ...
    if (query.hasInstructions != null && item.hasInstructions !== query.hasInstructions)
      return false
    if (query.minDuration != null || query.maxDuration != null) {
      if (!matchesNumberRange(item.durationMinutes, query.minDuration, query.maxDuration))
        return false
    }
...
export async function queryWorkoutLibrary(
  ...
  const filtered = sortRecords(applyLocalFilters(fetchedRecords, localFilterQuery), sort).slice(
    0,
    normalizedLimit,
  )
...
export async function commandWorkoutLibrary(flags, deps) {
  const { requirePositiveInteger, requireNumber, toBoolean } = deps
