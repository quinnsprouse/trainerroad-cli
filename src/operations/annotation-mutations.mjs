...
export async function commandAddAnnotation(flags, deps) {
  const { requireFlag, toBoolean, normalizeDateOnlyInput, requirePositiveInteger } = deps
  const dryRun = toBoolean(flags["dry-run"], false)
  const type = resolveAnnotationType(requireFlag("add-annotation", flags, "type"))
  if (!type) {
    throw new Error(
      `Invalid --type "${flags.type}". Expected one of: ${validTypeNames()}, or a numeric typeId.`,
    )
  }
  ...
  const title =
    flags.title !== undefined && flags.title !== null && String(flags.title) !== ""
      ? String(flags.title)
  ...
  for (const id of createdIds) {
    ...
    if (
      ...
      detail.dateOnly === date &&
      detail.durationSeconds === request.duration &&
      detail.title === title &&
      detail.text === text &&
  ...
  const annotation = candidates.length === 1 ? candidates[0] : null
