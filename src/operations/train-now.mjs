...
function summarizeWorkoutInfo(info) {
  ...
  return {
    ...
    durationMinutes: Number.isFinite(Number(info.durationInSeconds))
      ? Math.round(Number(info.durationInSeconds) / 60)
      : null,
    tss: info.tss ?? null,
...
function flattenSuggestions(suggestions, workoutInfoById, categoryFilter = null) {
  const categories =
    suggestions && typeof suggestions === "object" ? Object.entries(suggestions) : []
  const rows = []
...
async function requirePrivateMember(flags, deps) {
  ...
  try {
    ...
    throw new Error(
      "train-now requires private authenticated mode. Login first with trainerroad-cli login.",
    )
  }
}

export async function commandTrainNow(flags, deps) {
  const { requirePositiveInteger } = deps
  ...
  const records = flattenSuggestions(
