...
function summarizeLevels(levelsPayload) {
  return {
    ...
    athleteLevels:
      levelsPayload && typeof levelsPayload.athleteLevels === "object"
        ? levelsPayload.athleteLevels
        : {},
  }
}
...
function summarizeChart(chartData, pointLimit = 200) {
  ...
  const pointsUseMilliseconds =
    rawPoints.length > 1 &&
    Number(rawPoints[1]?.seconds) >= 1000 &&
    Number(rawPoints[1]?.seconds) % 1000 === 0
...
async function listPlannedWorkoutsOnDate(client, memberInfo, dateIso) {
  ...
  const candidateIds = (
    Array.isArray(timeline?.plannedActivities) ? timeline.plannedActivities : []
  )
    .filter((item) => {
...
export async function commandWorkoutDetails(flags, deps) {
  const { requireFlag, requirePositiveInteger, toBoolean } = deps
  ...
  const [byIdRows, summaryPayload, levelsPayload, chartData] = await Promise.all([
    ...
    includeChart
      ? client.getWorkoutChartData(workoutId, memberInfo.username)
...
export async function commandAddWorkout(flags, deps) {
  ...
  const matchingExisting = existingOnDate
    .filter((item) => {
      ...
      return (
        plannedWorkoutId === workoutId && (plannedOutside == null || plannedOutside === outside)
      )
    })
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    const candidates = afterTarget.filter(
      (item) => !beforeIds.has(item.id) && Number(item?.workout?.id) === Number(workoutId),
    )
    if (candidates.length > 1)
      throw new Error("Ambiguous added workout; inspect the calendar before retrying")
...
export async function commandCopyWorkout(flags, deps) {
  ...
  const mutation = await client.copyPlannedActivity(
    sourcePlannedActivityId,
    targetDate,
    memberInfo.username,
  )
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    const candidates = afterTarget.filter(
      (item) =>
        !beforeIds.has(item.id) && Number(item?.workout?.id) === Number(source?.workout?.id),
    )
    if (candidates.length > 1)
      throw new Error("Ambiguous copied workout; inspect the calendar before retrying")
    created = candidates[0]
    if (created) break
    await sleep(500)
  }
