...
import { commandRemoveWorkout } from "../src/commands/workout-mutations.mjs";
...
test("remove-workout deletes the planned activity and reports what it removed", async () => {
  const client = createClient({ existing: { p1: plannedWorkout() } });
  const { deps, outputs } = createDeps(client);

  await commandRemoveWorkout({ id: "p1", json: true }, deps);
...
test("remove-workout --dry-run previews without deleting", async () => {
  const client = createClient({ existing: { p1: plannedWorkout() } });
  const { deps, outputs } = createDeps(client);

  await commandRemoveWorkout({ id: "p1", "dry-run": true, json: true }, deps);
...
test("remove-workout is a no-op when the planned activity is already gone", async () => {
  const client = createClient();
  const { deps, outputs } = createDeps(client);

  await commandRemoveWorkout({ id: "missing", json: true }, deps);
...
test("remove-workout surfaces non-404 lookup failures", async () => {
  const client = createClient();
