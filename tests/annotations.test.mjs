...
} from "../src/operations/annotation-mutations.mjs"
...
await test("add-annotation posts a whole-day duration and reports the created annotation", async () => {
...
await test("add-annotation --days sets the duration and defaults the title to the type", async () => {
  const client = createClient()
  const { deps, outputs } = createDeps(client)

  outputs.push(
    await commandAddAnnotation(
      { type: "time-off", date: "2026-09-21", days: "3", json: true },
      deps,
    ),
  )
...
await test("add-annotation accepts aliases and numeric type ids", async () => {
  const client = createClient()
  ...
  const typeIds = client.calls
    .filter(([name]) => name === "createAnnotation")
    .map(([, body]) => body.typeId)
  assert.deepEqual(typeIds, [2, 3])
})

await test("add-annotation rejects unknown types and end dates before the start", async () => {
  const client = createClient()
  const { deps } = createDeps(client)
  ...
  await assert.rejects(
    () =>
      commandAddAnnotation(
        { type: "note", date: "2026-09-21", "end-date": "2026-09-20", json: true },
        deps,
      ),
    /is before --date/,
  )
...
await test("add-annotation --dry-run previews without calling the API", async () => {
  const client = createClient()
  const { deps, outputs } = createDeps(client)

  outputs.push(
    await commandAddAnnotation(
      { type: "note", date: "2026-09-21", title: "New saddle", "dry-run": true, json: true },
      deps,
    ),
  )
...
await test("annotation-details returns the compact detail shape", async () => {
  const client = createClient({ existing: ["ann-1"], detailById: { "ann-1": detail() } })
  const { deps, outputs } = createDeps(client)

  outputs.push(await commandAnnotationDetails({ id: "ann-1", json: true }, deps))
...
await test("remove-annotation deletes an existing annotation and reports what it removed", async () => {
  const client = createClient({ existing: ["ann-1"], detailById: { "ann-1": detail() } })
  const { deps, outputs } = createDeps(client)

  outputs.push(await commandRemoveAnnotation({ id: "ann-1", json: true }, deps))
...
await test("remove-annotation is a no-op when the annotation is already gone", async () => {
  const client = createClient()
  const { deps, outputs } = createDeps(client)

  outputs.push(await commandRemoveAnnotation({ id: "missing", json: true }, deps))
...
await test("remove-annotation --dry-run leaves the annotation in place", async () => {
  const client = createClient({ existing: ["ann-1"], detailById: { "ann-1": detail() } })
  const { deps, outputs } = createDeps(client)

  outputs.push(await commandRemoveAnnotation({ id: "ann-1", "dry-run": true, json: true }, deps))
