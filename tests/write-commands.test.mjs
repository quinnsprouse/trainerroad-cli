...
} from "../src/operations/workout-mutations.mjs"
import { commandAddWorkout, commandCopyWorkout } from "../src/operations/workout-tools.mjs"
...
await test("replace-workout returns a no-op when the alternate is already applied", async () => {
  let replaceCalls = 0
  const client = {
    async getMemberInfo() {
      return MEMBER_INFO
  ...
  outputs.push(await commandReplaceWorkout({ id: "p1", "alternate-id": "18128", json: true }, deps))

  assert.equal(replaceCalls, 0)
  assert.equal(outputs[0].noop, true)
})

await test("switch-workout returns a no-op when already in the requested mode", async () => {
  let switchCalls = 0
  const client = {
    async getMemberInfo() {
      return MEMBER_INFO
  ...
  outputs.push(await commandSwitchWorkout({ id: "p1", mode: "outside", json: true }, deps))

  assert.equal(switchCalls, 0)
  assert.equal(outputs[0].noop, true)
})

await test("add-workout dry-run warns about matching workouts but does not convert to a no-op", async () => {
  let addCalls = 0
...
await test("copy-workout dry-run warns about matching workouts but does not convert to a no-op", async () => {
  let copyCalls = 0
  ...
  outputs.push(
    await commandCopyWorkout(
      { id: "source", date: "2026-03-16", "dry-run": true, json: true },
      deps,
    ),
  )
...
await test("add-workout still performs the mutation when matching workouts already exist", async () => {
  let addCalls = 0
  let phase = "before"
  const existing = plannedActivity({
    id: "existing",
...
await test("copy-workout still performs the mutation when matching workouts already exist", async () => {
  let copyCalls = 0
  let phase = "before"
  const source = plannedActivity({
    id: "source",
  ...
  outputs.push(await commandCopyWorkout({ id: "source", date: "2026-03-16", json: true }, deps))
