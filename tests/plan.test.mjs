...
import { commandPlan } from "../src/operations/plan.mjs"
    ...
    deps: {
      ...
      async resolveQueryContext() {
        return {
          ...
          authenticatedMemberInfo: MEMBER_INFO,
        }
      },
...
function notFound(path) {
  return new HttpError(`Request failed: 404 Not Found for ${path} -> `, {
    status: 404,
...
await test("client builds plan-builder URLs from memberId and keeps username in the referer", async () => {
  const requests = []
...
await test("client surfaces HTTP status on failed requests", async () => {
  const originalFetch = globalThis.fetch
...
await test("plan command passes memberId (not username) to plan-builder client calls", async () => {
  const client = createClient({
    currentPlanError: notFound("/app/api/plan-builder/current-custom-plan/211199"),
  })
  ...
  assert.deepEqual(
    client.calls.toSorted((a, b) => String(a).localeCompare(String(b))),
    [
...
await test("plan command treats a current-custom-plan 404 as non-fatal and derives the current plan", async () => {
  const client = createClient({
    currentPlanError: notFound("/app/api/plan-builder/current-custom-plan/211199"),
  })
  const { deps, outputs } = createDeps(client)

  outputs.push(await commandPlan({ json: true, view: "current" }, deps))
...
await test("plan command still fails on non-404 current-custom-plan errors", async () => {
  const error = new HttpError("Request failed: 500 Server Error", {
    status: 500,
...
await test("plan command prefers the explicit current-custom-plan payload when available", async () => {
  const explicit = {
    id: 777,
    name: "Explicit Plan",
    memberId: 211199,
...
await test("plan command reports no current plan when today falls outside every plan window", async () => {
  const client = createClient({
    currentPlanError: notFound("/app/api/plan-builder/current-custom-plan/211199"),
  })
...
await test("deriveCurrentPlanFromPlans picks the plan whose date window contains today", () => {
...
await test("deriveCurrentPlanFromPlans treats window boundaries as inclusive", () => {
...
await test("deriveCurrentPlanFromPlans prefers the latest-starting plan when windows overlap", () => {
  const overlapping = [
    ...PLANS,
    {
      id: 503,
...
await test("deriveCurrentPlanFromPlans falls back to date containment when phases lack customPlanId", () => {
  const phasesWithoutIds = PHASES.map(({ customPlanId: _customPlanId, ...rest }) => rest)
  const derived = deriveCurrentPlanFromPlans(PLANS, phasesWithoutIds, TODAY)
  assert.equal(derived.id, 502)
  assert.deepEqual(
    derived.phases.map((phase) => phase.type),
    ["base", "build", "specialty"],
  )
  assert.equal(derived.currentPhase, "build")
})

await test("derived current plan names the active phase for agents", () => {
