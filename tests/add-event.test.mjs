...
import { commandAddEvent } from "../src/commands/event-mutations.mjs";
...
test("add-event posts a TSS-based event and reports the created record", async () => {
  const client = createClient();
  const { deps, outputs } = createDeps(client);

  await commandAddEvent(
    { name: "Test Gravel", date: "2027-02-01", discipline: "gravel", priority: "A", duration: "300", tss: "340", notes: "long day", json: true },
    deps,
  );
  ...
  assert.match(payload.message, /Added A event "Test Gravel" on 2027-02-01 \(plannedActivityId=evt-new\)/);
  assert.match(payload.message, /remove-workout --id evt-new/);
});

test("add-event uses intensity mode when --tss is absent and defaults priority to B", async () => {
  const client = createClient();
  const { deps, outputs } = createDeps(client);

  await commandAddEvent(
    { name: "Crit", date: "2026-10-06", discipline: "3", duration: "60", intensity: "9", json: true },
    deps,
  );
...
test("add-event validates discipline, priority, duration, and stress inputs", async () => {
  const client = createClient();
  const { deps } = createDeps(client);
  const base = { name: "X", date: "2026-10-06", discipline: "gravel", duration: "60", tss: "100", json: true };

  await assert.rejects(() => commandAddEvent({ ...base, discipline: "unicycle" }, deps), /Invalid --discipline/);
  await assert.rejects(() => commandAddEvent({ ...base, priority: "Z" }, deps), /Invalid --priority/);
  await assert.rejects(() => commandAddEvent({ ...base, duration: "0" }, deps), /--duration <minutes> is required/);
  await assert.rejects(() => commandAddEvent({ ...base, tss: undefined }, deps), /--tss <number> or --intensity/);
  await assert.rejects(() => commandAddEvent({ ...base, date: "next friday" }, deps), /Invalid --date/);
  assert.equal(client.calls.length, 0);
});

test("add-event --dry-run previews without calling the API", async () => {
  const client = createClient();
  const { deps, outputs } = createDeps(client);

  await commandAddEvent(
    { name: "Test Gravel", date: "2027-02-01", discipline: "gravel", duration: "300", tss: "340", "dry-run": true, json: true },
    deps,
  );
