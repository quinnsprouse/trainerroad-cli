...
await test("login posts JSON to /app/api/login/login and stores the auth cookie", async () => {
  ...
  await withFetch(
    ...
    async (calls) => {
      ...
      assert.deepEqual(result, {
        ok: true,
        redirect: "/app/career/quinnsprouse",
        hasAuthCookie: true,
      })
...
await test("login accepts a PascalCase success payload", async () => {
  const sessionFile = await tempSessionFile()
  await withFetch(
    () =>
      jsonResponse(
...
await test("login reports rejected credentials without falling back to the form flow", async () => {
  const sessionFile = await tempSessionFile()
  await withFetch(
...
await test("login falls back to the legacy form flow when the JSON route is gone", async () => {
  const sessionFile = await tempSessionFile()
  const formPage = `<form>
  ...
  await withFetch(
    (request) => {
      ...
      return new Response(null, {
        status: 302,
        headers: {
          location: "/app/career/quinnsprouse",
          "set-cookie": "SharedTrainerRoadAuth=legacy; path=/",
        },
      })
    },
    async (calls) => {
...
await test("login explains when neither flow is available", async () => {
  const sessionFile = await tempSessionFile()
  await withFetch(
    (request) =>
...
await test("every API request sends the camel-case format header by default", async () => {
  await withFetch(
    (request) => {
      assert.equal(request.headers.get("trainerroad-jsonformat"), "camel-case")
...
await test("PascalCase payloads are normalised to camelCase when the format header is ignored", async () => {
  await withFetch(
    () =>
      jsonResponse([
        {
...
await test("camelizeKeys leaves camelCase payloads untouched and looksPascalCase is conservative", () => {
...
await test("camelizeKeys normalises PascalCase objects nested inside a camelCase envelope", () => {
  const mixed = {
